        ...
        <table class="w-full text-sm text-left rtl:text-right text-gray-900 rounded-lg">
            ...
            <tbody>
                ...
                <tr class=" text-left text-xs even:bg-violet-50 odd:bg-white">
                    ...
                    <td scope="row" class="px-6 py-2">{{ $pelanggan->kategori_pelanggan->kategori_pelanggan}}</td>

                    <td class="text-center">
                        <a href="#"
                            @click.prevent="loadEditForm('{{ route('pelanggans.edit', $pelanggan->pelanggan_id) }}')"
                            class="text-indigo-600 hover:text-indigo-900"><i class="fa-solid fa-pen cursor-pointer text-amber-300 mx-1"></i></a>
                        <form action="{{ route('barangs.destroy', $pelanggan->pelanggan_id) }}" method="POST" style="display:inline;">
                            @csrf
                            @method('DELETE')
