       ...
       + Tambah Barang Baru
        ...
        <table class="w-full text-sm text-left rtl:text-right text-gray-900 rounded-lg">
            <thead class="text-xs text-gray-900 border-b-1 rounded-lg bg-neutral-">
                <tr>
                    <th scope="col" class="px-6 py-2 text-center">Kode Barang</th>
                    <th scope="col" class="px-6 py-2">Nama Barang</th>
                    <th scope="col" class="px-6 py-2">Nama Satuan</th>
                    <th scope="col" class="px-6 py-2">Kategori Barang</th>
                    <th scope="col" class="px-6 py-2">Supplier</th>
            ...
            <tbody>
                ...
                <tr class=" text-left text-xs even:bg-violet-50 odd:bg-white">
                    <td scope="row" class="px-6 py-2 text-center">{{ $barang->kode_barang }}</td>
                    <td scope="row" class="px-6 py-2">{{ $barang->nama_barang }}</td>
                    <td scope="row" class="px-6 py-2">{{ $barang->nama_satuan }}</td>
                    <td scope="row" class="px-6 py-2">{{ $barang->kategori->nama_kategori_barang }}</td>
                    <td scope="row" class="px-6 py-2">{{ $barang->supplier->namaSupplier ?? '-'}}</td>

                    <td class="text-center">
                        <a href="#"
                            @click.prevent="loadEditForm('{{ route('barangs.edit', $barang->kode_barang) }}')"
                            class="text-indigo-600 hover:text-indigo-900"><i class="fa-solid fa-pen cursor-pointer text-amber-300 mx-1"></i></a>
                        <form action="{{ route('barangs.destroy', $barang->kode_barang) }}" method="POST" style="display:inline;">
                            @csrf
                            @method('DELETE')
                            <button type="submit" onclick="return confirm('Yakin ingin menghapus data ini?')"><i class="fa-solid fa-trash cursor-pointer text-red-700 mx-1"></i></button>
                        </form>
                    </td>
                </tr>
                @endforeach
