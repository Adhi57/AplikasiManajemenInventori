        ...
        <div class="grid grid-cols-12 gap-4 items-center mb-6">
            <div class="relative col-span-12 md:col-span-8 text-gray-600">
                <form method="GET" action="{{ route('users.index') }}" class="flex flex-wrap items-center gap-3">
                    ...
                    <select name="role"
                        class="h-10 border border-gray-300 rounded-xl px-3 text-sm focus:ring-red-600 focus:border-red-600 shadow-sm outline-none">
                    ...
                    <button type="submit"
                        class="bg-red-700 text-white px-4 py-2 rounded-xl shadow-lg hover:bg-red-800 transition duration-150 ease-in-out">
                ...
                <div class="p-6">
                    ...
                    <div class="overflow-x-auto max-h-96 overflow-y-auto">
                        <table class="w-full text-sm text-left text-gray-700">
                            <thead
                                class="text-xs text-white uppercase bg-red-700 border-b-4 border-red-500 sticky top-0 z-10">
                                <tr>
                            ...
                            <tbody>
                                @foreach ($users as $user)
                                    <tr
                                        class="text-xs even:bg-red-50 odd:bg-white border-b hover:bg-red-100 transition duration-100">
                                        ...
                                        <td class="px-6 py-2 text-center">
                                            ...
                                            <select onchange="updateRole('{{ $user->user_id }}', this.value)"
                                                class="text-[10px] font-bold uppercase tracking-wider px-2 py-1 rounded-full border-none focus:ring-0 cursor-pointer {{ $roleBadgeClass }}">
                                                @foreach (['SuperAdmin', 'Admin', 'Head', 'Staff'] as $role)
                                        ...
                                        <td class="px-6 py-2 text-center whitespace-nowrap">
                                            <a href="{{ route('users.edit', $user->user_id) }}"
                                                class="inline-block text-gray-500 hover:text-amber-600 transition p-1">
                                                <i class="fa-solid fa-pen-to-square text-sm"></i>
                                            </a>

                                            <button type="button"
                                                onclick="confirmDelete('{{ $user->user_id }}', '{{ $user->nama_lengkap }}')"
                                                class="text-gray-500 hover:text-red-700 transition p-1">
